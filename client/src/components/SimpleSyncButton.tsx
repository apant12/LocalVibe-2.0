...
export default function SimpleSyncButton() {
  ...
  const ticketmasterSyncMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch('/api/sync/ticketmaster?location=San Francisco&limit=10');
      if (!response.ok) {
  ...
  const googlePlacesSyncMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch('/api/sync/places?location=San Francisco&limit=10');
      if (!response.ok) {
        throw new Error(`Failed to sync: ${response.status}`);
      }
      return response.json();
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/experiences"] });
      toast({
        title: "Google Places Sync Complete!",
        description: `Added ${data.count} new places from Google Places to your feed`,
      });
    },
    onError: (error: any) => {
      toast({
    ...
    <div className="p-4 space-y-4">
      <div className="space-y-3">
        <Button
          ...
          disabled={ticketmasterSyncMutation.isPending || googlePlacesSyncMutation.isPending}
        ...
        <Button
          onClick={() => googlePlacesSyncMutation.mutate()}
          disabled={ticketmasterSyncMutation.isPending || googlePlacesSyncMutation.isPending}
          className="w-full bg-green-600 hover:bg-green-700 text-white"
        >
          {googlePlacesSyncMutation.isPending ? (
            <div className="flex items-center space-x-2">
              <div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></div>
              <span>Syncing Google Places...</span>
            </div>
          ) : (
            <div className="flex items-center space-x-2">
              <MapPin className="w-4 h-4" />
              <span>Load Places from Google</span>
            </div>
          )}
        </Button>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {syncedEvents.slice(0, 5).map((event, index) => (
              <div key={index} className="bg-gray-800 rounded-lg p-3 border border-blue-600/20">
